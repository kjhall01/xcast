...
@pytest.mark.parametrize("reg,x,y", [(reg, *make_regtestdata()) for reg in xc.all_regressors])
def test_regressors(reg,x,y):
	if reg in [xc.rPoissonRegression, xc.rGammaRegression]:
		reg = reg()
...
@pytest.mark.parametrize("clf,x,y", [(clf, *make_clftestdata()) for clf in xc.arbitrary_classifiers])
...
@pytest.mark.parametrize("clf,x,y", [(clf, *make_clftestdata()) for clf in xc.tercile_classifiers])
