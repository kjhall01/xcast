...
class SpatialPCA:
    def __init__(self, latitude_weighting=False, preprocessing='std', **kwargs):
        self.kwargs = kwargs
    ...
    def fit(self, X, x_lat_dim=None, x_lon_dim=None, x_sample_dim=None, x_feature_dim=None, ):
        ...
        if self.preprocessing == 'std': 
            X = ( X - X.mean(x_sample_dim)) / X.std(x_sample_dim)


        if self.latitude_weighting:
            # integrated cosine weighting for X (predictors)
            average_latitude_width = np.sqrt(getattr(X, x_lat_dim).diff(x_lat_dim).mean() ** 2) * 0.5
            R1 = getattr(X, x_lat_dim) - average_latitude_width
            R2 = getattr(X, x_lat_dim) + average_latitude_width
            xweights = (np.sin(R1/180.0*np.pi) -
                        np.sin(R2/180.0*np.pi)) / (R1 - R2)
            xweights = np.abs(xweights)
            self.xweights = np.sqrt(xweights)
            X2 = X * self.xweights
        else:
            X2 = X
    ...
    def transform(self, X, x_lat_dim=None, x_lon_dim=None, x_sample_dim=None, x_feature_dim=None, ):
        ...
        for i in range(self.shape[-1]):
            data = X2.isel(**{x_feature_dim: i}).stack(point=(x_lat_dim, x_lon_dim)
