...
class SpatialPCA:
	...
	def fit(self, X, x_lat_dim=None, x_lon_dim=None, x_sample_dim=None, x_feature_dim=None, feat_chunks=1, samp_chunks=1, verbose=False ):
		x_lat_dim, x_lon_dim, x_sample_dim, x_feature_dim = guess_coords(X, x_lat_dim, x_lon_dim, x_sample_dim, x_feature_dim)
		check_all(X, x_lat_dim, x_lon_dim, x_sample_dim, x_feature_dim)
	...
	def _apply_fit_to_chunk(self, X1, feat_ndx_low, samp_ndx_low, x_lat_dim=None, x_lon_dim=None, verbose=False):
		x_data = X1.values
	...
	def transform(self, X, x_lat_dim=None, x_lon_dim=None, x_sample_dim=None, x_feature_dim=None, feat_chunks=1, samp_chunks=1 , verbose=False, override=False, destination='.xcast_worker_space'):
		...
		attrs = X.attrs 
		r = xr.DataArray(data=results, coords=coords, dims=dims, attrs=attrs)
		r.attrs['generated_by'] =  attrs['generated_by'] + '\n  XCAST Spatial PCA Transform' if 'generated_by' in attrs.keys() else '\n  XCAST Spatial PCA Transform'
		return r 

	def _apply_transform_to_chunk(self, X1, feat_ndx_low, samp_ndx_low, x_lat_dim=None, x_lon_dim=None, verbose=False):
		x_data = X1.values
		results = []
		for i in range(x_data.shape[0]):
			results.append([])
	...
	def eofs(self):
		...
		ret = xr.DataArray(data=eofs, coords=coords, dims=[self.feature_dim, 'MODE', self.lat_dim, self.lon_dim ], attrs=attrs)
