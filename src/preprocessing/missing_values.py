...
def fill_time_mean(X, x_lat_dim='Y', x_lon_dim='X', x_sample_dim='T', x_feature_dim='M', missing_value=np.nan):
	...
	assert xr.ufuncs.isnan(filled).sum().values == 0, 'Found empty space/feature slice- at least one point in at least one feature is missing data in all samples '
	return filled
...
def fill_space_mean(X, x_lat_dim='Y', x_lon_dim='X', x_sample_dim='T', x_feature_dim='M', missing_value=np.nan):
	...
	assert xr.ufuncs.isnan(filled).sum().values == 0, 'Found empty sample/feature slice - at least one feature for at least one sample is missing all data across lat/lon '
	return filled
