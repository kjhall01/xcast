...
def open_XrDataset(filename, use_dask=False):
	...
	try:
		ds = xr.open_dataset(str(Path(filename).absolute()))
	except:
		ds = xr.open_dataset(str(Path(filename).absolute()), decode_times=False)
	return ds
