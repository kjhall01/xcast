...
class EinsteinLearningMachine:
    """Ensemble Extreme Learning Machine using Einstein Notation"""

    def __init__(self, activation='lin', hidden_layer_size=5, c=3, preprocessing='minmax', n_estimators=30):
    ...
    def fit(self, x, y):
        ...
        for i in range(self.n_estimators):
            ...
            if info > 0:
                ...
                B = np.linalg.lstsq(hth_plus_ic[i,:,:], ht[i,:,:])[0]
            self.betas.append(B)
            ...
            if info > 0:
                hth_plus_ic = hth_plus_ic + np.triu(hth_plus_ic, k=1).T
                B = np.linalg.lstsq(hth_plus_ic[i,:,:], ht_logs[i,:,:])[0]
