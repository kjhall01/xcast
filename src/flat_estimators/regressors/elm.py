...
class ELMRegressor:
	...
	def __init__(self, activation='sigm', hidden_layer_size=5, initialization='random', pruning='none', pca=-999, c=1, preprocessing='none', dropconnect_pr=-1.0, dropout_pr=-1.0, verbose=False):
		...
		assert activation in ['sigm', 'tanh', 'relu', 'lin', 'rbf_l1', 'rbf_l2', 'rbf_linf'], 'invalid activation function {}'.format(activation)
		...
		if self.pruning == 'pca':
			...
			for i in range(len(scores)):
				...
				if x.dtype == np.float64 and y.dtype == np.float64:
					_, B, info = la.dposv(hth_plus_ic, ht)
				elif x.dtype == np.float32 and y.dtype == np.float32:
					_, B, info = la.sposv(hth_plus_ic, ht)
				else:
					assert False, 'x: {} and y: {} not matching or good dtypes for lapack'.format(x.dtype ,y.dtype )
				if info > 0:
