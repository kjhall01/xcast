	...
	for i in range(x_data.shape[0]):
		...
		for j in range(x_data.shape[1]):
			...
			if np.isnan(np.min(x_train)) or np.isnan(np.min(y_train)):
				...
				x = func1(x_train, y_train, **kwargs)
				x = reformat_vector(x, 'col')
				ret[i].append(x)
		ret[i] = np.asarray(ret[i]).astype(float)
	return np.asarray(ret).astype(float)

def metric(func):
	def func1(X, Y, x_lat_dim='Y', x_lon_dim='X', x_sample_dim='T', x_feature_dim='M', y_lat_dim='Y', y_lon_dim='X', y_sample_dim='T', y_feature_dim='M', lat_chunks=1, lon_chunks=1,  feat_chunks=1, samp_chunks=1, verbose=False, rechunk=True, parallel_in_memory=True, n=1, xfmt='col', yfmt='col', **kwargs):
		...
		if rechunk:
			X1 = X1.chunk({x_lat_dim: max(X1.shape[list(X1.dims).index(x_lat_dim)] // lat_chunks,1), x_lon_dim: max(X1.shape[list(X1.dims).index(x_lon_dim)] // lon_chunks,1)})
			Y1 = Y1.chunk({y_lat_dim: max(Y1.shape[list(Y1.dims).index(y_lat_dim)] // lat_chunks, 1), y_lon_dim: max(Y1.shape[list(Y1.dims).index(y_lon_dim)] // lon_chunks,1)})
