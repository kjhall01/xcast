...
class ExtremeLearningMachine:
	...
	def fit(self, X, Y, x_coords={'X':'X', 'Y':'Y', 'T':'T', 'M':'M'}, y_coords={'X':'X', 'Y':'Y', 'T':'T'}, verbose=False, rescale_x='NORMAL', rescale_y='NORMAL', pca_x=False, fill='mean', t_chunks=20, x_chunks=10, y_chunks=10):
		self.model.fit(X, Y, ELM_, x_coords=x_coords, y_coords=y_coords, verbose=verbose, rescale_x=rescale_x, rescale_y=rescale_y, pca_x=pca_x, fill=fill, t_chunks=t_chunks, x_chunks=x_chunks, y_chunks=y_chunks)

	def predict(self, X, x_coords={'X':'X', 'Y':'Y', 'T':'T', 'M':'M'}, verbose=False, fill='mean', t_chunks=20, x_chunks=10, y_chunks=10):
		return self.model.predict( X, x_coords=x_coords, verbose=verbose, fill=fill, t_chunks=t_chunks, x_chunks=x_chunks, y_chunks=y_chunks)
...
class ELM_PCA:
	...
	def fit(self, X, Y, x_coords={'X':'X', 'Y':'Y', 'T':'T', 'M':'M'}, y_coords={'X':'X', 'Y':'Y', 'T':'T'}, verbose=False, rescale_x='NORMAL', rescale_y='NORMAL', pca_x=True, fill='mean', t_chunks=20, x_chunks=10, y_chunks=10):
		self.model.fit(X, Y, ELM_, x_coords=x_coords, y_coords=y_coords, verbose=verbose, rescale_x=rescale_x, rescale_y=rescale_y, pca_x=pca_x, fill=fill, t_chunks=t_chunks, x_chunks=x_chunks, y_chunks=y_chunks)
...
class EnsembleMean:
	...
	def fit(self, X, Y, x_coords={'X':'X', 'Y':'Y', 'T':'T', 'M':'M'}, y_coords={'X':'X', 'Y':'Y', 'T':'T'}, verbose=False, rescale_x='NONE', rescale_y='NONE', pca_x=False, fill='mean', t_chunks=20, x_chunks=10, y_chunks=10):
		self.model.fit(X, Y, EnsembleMean_, x_coords=x_coords, y_coords=y_coords, verbose=verbose, rescale_x=rescale_x, rescale_y=rescale_y, pca_x=pca_x, fill=fill, t_chunks=t_chunks, x_chunks=x_chunks, y_chunks=y_chunks)
...
class MultipleLinearRegression:
	...
	def fit(self, X, Y, x_coords={'X':'X', 'Y':'Y', 'T':'T', 'M':'M'}, y_coords={'X':'X', 'Y':'Y', 'T':'T'}, verbose=False, rescale_x='NORMAL', rescale_y='NORMAL', pca_x=False, fill='mean', t_chunks=20, x_chunks=10, y_chunks=10):
		self.model.fit(X, Y, LinearRegression_, x_coords=x_coords, y_coords=y_coords, verbose=verbose, rescale_x=rescale_x, rescale_y=rescale_y, pca_x=pca_x, fill=fill, t_chunks=t_chunks, x_chunks=x_chunks, y_chunks=y_chunks)
...
class SpatialPCR:
	def __init__(self, n_components=1,  **kwargs):
		assert False, 'Spatial PCA not yet available for big datasets'
		self.kwargs = kwargs
		self.kwargs['n_components'] = n_components
		self.n_components = n_components
		self.model = PointWiseMME(**self.kwargs)

	def fit(self, X, Y, x_coords={'X':'X', 'Y':'Y', 'T':'T', 'M':'M'}, y_coords={'X':'X', 'Y':'Y', 'T':'T'}, verbose=False, rescale_x='None', rescale_y='NORMAL', pca_x=True, fill='mean', t_chunks=20, x_chunks=10, y_chunks=10):
...
class SM_PCR:
	def __init__(self, n_components=1,  **kwargs):
		assert False, 'SM_PCA not supported for big datasets yet'
		self.kwargs = kwargs
		self.kwargs['n_components'] = n_components
		self.n_components = n_components
		self.model = PointWiseMME(**self.kwargs)
