...
def validate(mme, X, Y, x_coords={'X':'X', 'Y':'Y', 'T':'T', 'M':'M'}, y_coords={'X':'X', 'Y':'Y', 'T':'T', 'M':'M'}, method='cross', window=3, initial_training_period=None, training_period_step=None, nd=1, missing_value=-1, fill='mean', save_stepwise_hindcasts=False, save_stepwise_skill=False, verbose=0,  pca_x=False,  n_components=5, rescale_x=None, rescale_y=None, regrid_to='Y', t_chunks=20, x_chunks=10, y_chunks=10, **kwargs):
	...
	#print('{} Starting Regrid-Fill'.format(dt.datetime.now()))
	if str(regrid_to).upper() == 'Y':
		X, Y = regrid_fill(X, Y, x_coords, y_coords, missing_value=missing_value, verbose=verbose, fill=fill, t_chunks=t_chunks, x_chunks=x_chunks, y_chunks=y_chunks)
	else:
		Y, X = regrid_fill(Y, X, y_coords, x_coords, missing_value=missing_value, verbose=verbose, fill=fill, t_chunks=t_chunks, x_chunks=x_chunks, y_chunks=y_chunks)
	...
	while x_train is not None and y_train is not None:
		...
		if str(rescale_y).upper() == 'NORMAL':
			...
			scaler_y.fit(y_train, y_coords, verbose=verbose)
		...
		if str(rescale_x).upper() == 'NORMAL':
			...
			scaler_x.fit(x_train, x_coords, verbose=verbose)
		...
		for j in range(nd):
			...
			val_mme.fit(x_train, y_train, x_coords=x_coords, y_coords=y_coords, rescale_x='NONE', rescale_y='NONE', pca_x=False, verbose=verbose, x_chunks=x_chunks, y_chunks=y_chunks, t_chunks=t_chunks)
			#train = val_mme.predict(x_train, x_coords=x_coords, verbose=verbose)
			val = val_mme.predict(x_test, x_coords=x_coords, verbose=verbose, x_chunks=x_chunks, y_chunks=y_chunks, t_chunks=t_chunks)
			...
			hindcasts[i-1].append(getattr(val, varname).data)
		x_train, x_test = x_validator.get_next_window()
		y_train, y_test = y_validator.get_next_window()

	if verbose == 1:
		print('{} {} Validation for {}: ['.format(dt.datetime.now(), method.upper(), mme) + '*'*25 + '] 100% ({}/{})'.format( total, total) )
	hindcasts = da.concatenate(hindcasts, axis=-1)
	...
	averaged = da.nanmean(hindcasts, axis=0)
	preds = xr.Dataset({'predictions': ([ x_coords['Y'], x_coords['X'], x_coords['T']], averaged)}, coords=coords)
	ioa = index_of_agreement(preds, Y,  x_coords=x_coords_slice, y_coords=y_coords_slice, verbose=verbose, x_chunks=x_chunks, y_chunks=y_chunks, t_chunks=t_chunks).skill_measure.isel(M=0)
	pearson_coef = pearson_coefficient(preds, Y,  x_coords=x_coords_slice, y_coords=y_coords_slice, verbose=verbose, x_chunks=x_chunks, y_chunks=y_chunks, t_chunks=t_chunks).skill_measure.isel(M=0)
	pearson_p = pearson_significance(preds, Y,  x_coords=x_coords_slice, y_coords=y_coords_slice, verbose=verbose, x_chunks=x_chunks, y_chunks=y_chunks, t_chunks=t_chunks).skill_measure.isel(M=0)
	spearman_coef = spearman_coefficient(preds, Y,  x_coords=x_coords_slice, y_coords=y_coords_slice, verbose=verbose, x_chunks=x_chunks, y_chunks=y_chunks, t_chunks=t_chunks).skill_measure.isel(M=0)
	spearman_p = spearman_significance(preds, Y,  x_coords=x_coords_slice, y_coords=y_coords_slice, verbose=verbose, x_chunks=x_chunks, y_chunks=y_chunks, t_chunks=t_chunks).skill_measure.isel(M=0)
	rmse = root_mean_squared_error(preds, Y,  x_coords=x_coords_slice, y_coords=y_coords_slice, verbose=verbose, x_chunks=x_chunks, y_chunks=y_chunks, t_chunks=t_chunks).skill_measure.isel(M=0)
	mse = mean_squared_error(preds, Y,  x_coords=x_coords_slice, y_coords=y_coords_slice, verbose=verbose, x_chunks=x_chunks, y_chunks=y_chunks, t_chunks=t_chunks).skill_measure.isel(M=0)
	mae = mean_absolute_error(preds, Y, x_coords=x_coords_slice, y_coords=y_coords_slice, verbose=verbose, x_chunks=x_chunks, y_chunks=y_chunks, t_chunks=t_chunks).skill_measure.isel(M=0)

	data_vars = {
		'hindcasts': ([x_coords['Y'], x_coords['X'], x_coords['T'] ], averaged), # axis = 1 takes mean over ND
