...
from ..preprocessing import regrid, RankedTerciles
...
class cMemberCount:
	...
	def fit(self, X, Y, x_lat_dim=None, x_lon_dim=None, x_sample_dim=None, x_feature_dim=None, y_lat_dim=None, y_lon_dim=None, y_sample_dim=None, y_feature_dim=None, an_thresh=0.67, bn_thresh=0.33,  explicit=False ):
		...
		if len(self.regrid_coords_lat)*len(self.regrid_coords_lon) > 1:
			X1 = regrid(X1, self.regrid_coords_lon, self.regrid_coords_lat, x_lat_dim=x_lat_dim, x_lon_dim=x_lon_dim, x_sample_dim=x_sample_dim, x_feature_dim=x_feature_dim)
		...
		for i in range(X1.shape[list(X1.dims).index(x_feature_dim)]):
			...
			self.onehots.append(RankedTerciles(low_thresh=bn_thresh, high_thresh=an_thresh, explicit=explicit))
			self.onehots[i].fit(to_transform, x_lat_dim, x_lon_dim, x_sample_dim, x_feature_dim)
	...
	def predict(self, X, x_lat_dim=None, x_lon_dim=None, x_sample_dim=None, x_feature_dim=None ):
		...
		for i in range(X1.shape[list(X1.dims).index(x_feature_dim)]):
			...
			to_transform = to_transform.expand_dims(x_feature_dim)
			to_transform.coords[x_feature_dim] = [i]
			transformed_da = self.onehots[i].transform(to_transform, x_lat_dim, x_lon_dim, x_sample_dim, x_feature_dim)
...
class cMultivariateLogisticRegression(BaseClassifier):
	def __init__(self, **kwargs):
		super().__init__(**kwargs)
		self.model_type = MultivariateELRClassifier


class cExtendedLogisticRegression(BaseClassifier):
	def __init__(self, **kwargs):
		super().__init__(**kwargs)
